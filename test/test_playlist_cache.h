...
void print_playlists(const std::vector<Playlist> &playlists, const std::string
&name) { std::ostream_iterator<Playlist> playlist_out(std::cout, "\n");
        std::cout << name << '\n';
        std::copy(playlists.begin(), playlists.end(), playlist_out);
}

std::vector<Playlist> parse_playlists_from_cache(std::filesystem::path
cache_dir) { PlaylistCacheNode node;
        {
...
void test_update_empty_untracked() {
        ...
        std::vector<Playlist> playlists =
parse_playlists_from_cache(untracked_dir);
...
void test_update_untracked_unchanged() {
        std::vector<Playlist> expected_playlists = {{"id1", "etag1", "title1",
true, 1}}; UntrackedCache untracked(Platform::TEST);
        untracked.update(expected_playlists);
        untracked.update(expected_playlists);
