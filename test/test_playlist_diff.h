...
void test_subtract_song_hashes_added_remove() {
    ...
    bool failed = diff.added.size() != 1 || diff.added.count(1234) != 1 ||
                  diff.added[1234] != 1 || diff.removed.size() != 1 ||
                  diff.removed.count(4321) != 1 || diff.removed[4321] != 1;
    if (failed) {
...
void test_subtract_song_hashes_only_added() {
    ...
    bool failed = diff.added.size() != 2 || !diff.added.count(1234) ||
                  diff.added[1234] != 2 || !diff.added.count(4321) ||
                  diff.added[4321] != 1 || !diff.removed.empty();
    if (failed) {
...
void test_subtract_song_hashes_only_removed() {
    ...
    bool failed = diff.removed.size() != 2 || !diff.removed.count(1234) ||
                  diff.removed[1234] != 2 || !diff.removed.count(4321) ||
                  diff.removed[4321] != 1 || !diff.added.empty();
    if (failed) {
...
void test_addition_playlist_diff() {
    PlaylistDiff diff1 = {/*added=*/{{1, 1}, {2, 1}},
                          /*removed=*/{{11, 1}, {12, 2}}};
    PlaylistDiff diff2 = {/*added=*/{{2, 2}, {3, 1}},
                          /*removed=*/{{12, 1}, {13, 1}}};
    PlaylistDiff diff3 = diff1;
    diff3 += diff2;

    PlaylistDiff expected = {/*added=*/{{1, 1}, {2, 2}, {3, 1}},
                             /*removed=*/{{11, 1}, {12, 2}, {13, 1}}};
    if (diff3 != expected) {
