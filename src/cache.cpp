...
static inline fs::path head_path(Platform plat) { return playlist_cache_dir(plat) / "HEAD"; }
...
void save_node(const Node& node, Platform plat) {
	proto::CacheNode tmp;
	auto proto_node = create_proto_node(&node);
	std::fstream f(playlist_cache_dir(plat) / node.playlist.id, std::ios::binary|std::ios::out|std::ios::in);
...
void create_node(const Node& node, Platform plat) {
	...
	std::fstream file(playlist_cache_dir(plat) / next, std::ios::binary|std::ios::in|std::ios::out);
