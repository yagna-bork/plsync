...
long SpotifyAPI::paginated_GET(const std::string& endpoint,
                               nlohmann::json& initial_page, Params& params,
                               std::string& etag) {
...
bool SpotifyAPI::get_playlists(std::vector<Playlist>& playlists,
                               std::string& etag) {
...
SpotifyAuthAPI::refresh_access_tkn(const std::string& refresh_tkn) {
    std::vector<std::pair<std::string, std::string>> fields = {
        {"client_id", get_setting("client_id", platform)},
